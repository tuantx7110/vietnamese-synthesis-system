...
int psola::get_max_abs_value(int *a, int number){
	int res = 0;
	for(int i = 0; i < number; i++){
		res = max(res, abs(a[i]));
	}
...
bool psola::create_phone(diphone dip1, diphone dip2, phone &P, int * frame_position, int number_frames){
	...
	if(type_cn_flag){
		...
		dstart_coef = get_max_abs_value((int *)dip1.buffer, dip1.dw_diplen / 2);
		dfinish_coef = get_max_abs_value((int *)dip2.buffer, dip2.dw_diplen / 2);
...
//bool psola::create_syllable(syllable syl){
//
//}
