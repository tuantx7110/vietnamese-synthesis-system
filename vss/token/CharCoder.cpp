...
void CharCoder::UTF8Encode2BytesUnicode(char *output, const wchar_t *input)
{
	int j=0;
...
void CharCoder::UTF8Decode2BytesUnicode(wchar_t *output, const char *input)
{
	int j=0;
...
bool CharCoder::isLower(wchar_t input){
	for(int i = 0; i < 93; i++){
...
bool CharCoder::isUpper(wchar_t input){
	for(int i = 0; i < 93; i++){
...
int CharCoder::indexLower(wchar_t input){
	for(int i = 0; i < 93; i++){
		if(input == LowerPrecomposedUnicode[i])
			return i;
	}
...
int CharCoder::indexUpper(wchar_t input){
	for(int i = 0; i < 93; i++){
		if(input == UpperPrecomposedUnicode[i])
			return i;
	}
...
void CharCoder::readSeparator(char * input){
	UTF8Decode2BytesUnicode(sep, input);
}

bool CharCoder::isSeparator(wchar_t input){
	for(int i = 0; i < (int)wcslen(sep); i++) if(input == sep[i])
		return true;
	return false;
}

vector<string> CharCoder::processString(string str){
	vector<string> res;
